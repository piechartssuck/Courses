...
2. Click on "Download R for `XXX`", where `XXX` is either Mac, Linux, or Windows:

    ```{r, echo=FALSE, out.width="60%"}
    include_graphics("/img/install/install-r-links.png", error = FALSE)
    ```
    As of this writing, the current version of R is 4.0.3 (Bunny-Wunnies Freak Out - yup that's the name of the release because why not?) but if there is an update, please download tthat instead.
...
4. **This step is only for those with a Mac. If you have a Linux of Windows based machine, you can start installing RStudio**. macOS requires [download and install XQuartz](https://www.xquartz.org/){target="_blank"}. 
