...
```{r eval = TRUE, echo = FALSE, out.width= "45%", fig.align="center"}
include_graphics("/slides/Week3/install_packages.png", error = FALSE)
```
...
voter_fraud <- read_csv("2012 Voter Fraud.csv")
```
...
```{r echo = TRUE, eval = TRUE, echo = FALSE, out.width= "30%", fig.align="left"}
knitr::include_graphics("/slides/Week3/freq_secondary_output.png")
```
...
What do you think is happening? Think about it for a minute before moving on. The thing about R as opposed to the click and run nature of say SPSS is that you are forced to know exactly what command is being run and what its doing to your data. Because of this, you are less likely to make a mistake or run a statistical analysis that runs but is garbage.
...
The `?` symbol prior to any command will give you information about that function provided you have that package loaded.  Yup its not clearly not written in plain English but it does tell you that the `mode` command means something else in R. 

OK let's get back to the mode. To get the mode, we actually have to tell R how to go about calculating it. Below is a function that will help. In order for R to recognize it, you have to run the following sequence before using the command.
